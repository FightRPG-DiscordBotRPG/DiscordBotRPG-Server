...
class LeaderboardCraftLevel extends Leaderboard {
    ...
    async getPlayerRank() {
        let res = await conn.query("WITH ranked_orders AS (SELECT idCharacter, RANK() OVER (ORDER BY characterscraftlevel.actualLevel DESC, characterscraftlevel.actualExp) as 'rnk' FROM characterscraftlevel) SELECT rnk as 'rank' FROM ranked_orders WHERE idCharacter = ?;", [this.id]);
